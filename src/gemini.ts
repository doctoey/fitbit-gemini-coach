...
ข้อกำหนดที่เข้มงวดเพื่อความ Minimal แต่อ่านง่าย:
...
- ห้ามใส่หัวข้อใหญ่ และห้ามใช้ Markdown ตัวหนา (เช่น **จุดที่ทำได้ดี:**) หรือเครื่องหมายขีดคั่นใดๆ ทั้งสิ้น
- การแบ่งวรรคตอน: ให้แยกเนื้อหาแต่ละส่วน (คำทักทาย, จุดที่ทำได้ดี, การวิเคราะห์, คำแนะนำ, กำลังใจ) ออกจากกันด้วยการขึ้นบรรทัดใหม่ธรรมดา (Paragraph) เพื่อให้มีช่องไฟสบายตา
- เฉพาะในส่วนของ "คำแนะนำ": ให้เขียนสรุปสั้นๆ แยกเป็นบรรทัดใหม่ทีละข้ออย่างชัดเจน โดยไม่ต้องมีตัวเลขหรือแดช (Dash) นำหน้า เพื่อให้ยังอ่านง่ายเป็นเช็คลิสต์แต่คงความคลีน
- ในส่วนการวิเคราะห์การนอนหลับ ให้ระบุช่วงเวลาเข้านอนและตื่นนอน (เช่น เข้านอนเวลา XX:XX น. และตื่นนอนเวลา YY:YY น.) จากข้อมูลช่วงเวลานอนหลับที่ให้ไว้ด้วยทุกครั้ง
...
export function buildWeeklyPrompt(weeklyData: HealthData[]): string {
  ...
  const daysWithSleep = weeklyData.filter(
    (d) => d.sleepDurationMinutes > 0,
  ).length;
  const totalSleepMins = weeklyData.reduce(
    (sum, d) => sum + d.sleepDurationMinutes,
    0,
  );
  const avgSleepMins =
    daysWithSleep > 0 ? Math.round(totalSleepMins / daysWithSleep) : 0;
